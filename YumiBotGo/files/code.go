...
func newConversation(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	fmt.Println(cp.Part == nil)
	note :=msg.Data.Item.ConversationPart.Part[0].Body
	
	isMessageANote:=false
	note = strings.ToLower(note)
	/*note = strings.Replace(note, "{<p>", "", -1)
	note = strings.Replace(note, "</p>}", "", -1)*/
	isMessageANote = false
	

	if note == "yumi run buildium" || note == "yumi help"{
		isMessageANote = true
	} else{
	...
	if userType == "user" || isMessageANote == true{
		go makeAndSendNote(userId, conversationId, isMessageANote, note)
	}
	w.WriteHeader(http.StatusOK)
	w.Write([]byte("Received"))
}

func makeAndSendNote(ID string, conversationID string, isMessageANote bool, noteBody string) {
	p := fmt.Println
	...
	if isMessageANote == true && strings.Contains(noteBody,"yumi help"){
		_, err = ic.Conversations.Reply(conversationID, intercom.Admin{ID: "207278"}, intercom.CONVERSATION_NOTE, "Yumi has following commands right now: \n - run buildium for buildium auto responder")
		return
	...
	if isMessageANote == true && strings.Contains(noteBody,"yumi run buildium"){
		_, err = ic.Conversations.Reply(conversationID, intercom.Admin{ID: "207278"}, intercom.CONVERSATION_COMMENT, "From note\n\n" +buildiumMessage)
		return
